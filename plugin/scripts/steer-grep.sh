...
refs=$(jq -n --arg r project --arg s "$pattern" '{repo_id:$r, symbol:$s}' \
...
jq -n --arg s "$pattern" --arg n "$n" '{
  hookSpecificOutput: {
    hookEventName: "PreToolUse",
    permissionDecision: "deny",
    permissionDecisionReason: ("The efficient codebase graph already knows `\($s)` (\($n) caller/callee edges). Use the MCP tool find_references(repo_id=\"project\", symbol=\"\($s)\") for callers/callees, or lookup_symbol(query=\"\($s)\", repo_id=\"project\") for its definition, instead of grep.")
  }
}'
exit 0
