...
async def test_cache_lookup_exact_hit(client, efficient_db):
    prompt = "hello world"
...
async def test_cache_lookup_exact_hit_increments_hit_count(client, efficient_db):
    prompt = "increment me"
    prompt_hash = hashlib.sha256(prompt.encode()).hexdigest()
    await efficient_db[CACHE_ENTRIES].insert_one({
        "prompt_hash": prompt_hash, "embedding": [0.1] * 1024, "prompt_preview": prompt,
...
async def test_cache_store_endpoint_writes(client, efficient_db):
    ...
    count = await efficient_db[CACHE_ENTRIES].count_documents({})
    assert count == 1
    doc = await efficient_db[CACHE_ENTRIES].find_one({})
