...
async def test_metrics_with_cache_entries(client, efficient_db):
    now = datetime.now(timezone.utc)
    await efficient_db[CACHE_ENTRIES].insert_many([
...
async def test_metrics_with_compression_stats(client, efficient_db):
    now = datetime.now(timezone.utc)
    await efficient_db[COMPRESSION_STATS].insert_many([
...
async def test_metrics_combined_cache_and_compression(client, efficient_db):
    now = datetime.now(timezone.utc)
...
async def test_record_module_events_inserts(efficient_db):
    await record_module_events(efficient_db, [
    ...
    count = await efficient_db[REQUEST_LOG].count_documents({})
    assert count == 2
...
async def test_metrics_includes_augmenter_modules(client, efficient_db):
    now = datetime.now(timezone.utc)
    await efficient_db[REQUEST_LOG].insert_many([
...
async def test_metrics_augmenters_and_cache_combined(client, efficient_db):
    now = datetime.now(timezone.utc)
